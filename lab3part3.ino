...
void loop() {
  ...
  while(1) {                          // the main continuous loop of the program.
    ...
    if(EEPROM.get(10, b)) {           // if the watchdog timed out and the flag was stored in EEPROM.
      if(b = 1) {
        b = 0;
        EEPROM.put(10, b);
...
ISR(WDT_vect) {
  ...
  byte b = 1;
  EEPROM.put(10, b);   // set the flag in EEPROM indicating a timeout to the rest of the system.
  wdt_reset();
